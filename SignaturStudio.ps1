...
$UpdatePreview = {
    ...
    if ($chkFirma.Checked -and $textboxes[7].Text.Trim() -ne "") {
        $preview.SelectionFont = [System.Drawing.Font]::new("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
        $preview.AppendText("$($textboxes[7].Text)`n")
    }
    
    $preview.SelectionFont = [System.Drawing.Font]::new("Segoe UI", 10)
    $addressParts = @()
    if ($chkStrasse.Checked -and $textboxes[8].Text.Trim() -ne "") { $addressParts += $textboxes[8].Text.Trim() }
    if ($chkPlz.Checked -and $textboxes[9].Text.Trim() -ne "") { 
        if ($chkOrt.Checked -and $textboxes[10].Text.Trim() -ne "") {
            $addressParts += "$($textboxes[9].Text.Trim()) $($textboxes[10].Text.Trim())"
        } else {
            $addressParts += $textboxes[9].Text.Trim()
        }
    }
    if ($chkLand.Checked -and $textboxes[11].Text.Trim() -ne "") { $addressParts += $textboxes[11].Text.Trim() }
    if ($addressParts.Count -gt 0) { $preview.AppendText(($addressParts -join " | ") + "`n") }
    
    if ($chkWebseite.Checked -and $textboxes[12].Text.Trim() -ne "") {
        $preview.AppendText("$($textboxes[12].Text)`n")
    }
...
$btnSave.Add_Click({
    ...
    try {
        ...
        if ($chkFirma.Checked -and $textboxes[7].Text.Trim() -ne "") {
            $htmlBody += "<p style='font-weight: bold;'>$($textboxes[7].Text)</p>"
        }
        ...
        if ($chkLand.Checked -and $textboxes[11].Text.Trim() -ne "") { $addressParts += $textboxes[11].Text.Trim() }
        if ($addressParts.Count -gt 0) { $htmlBody += "<p>$($addressParts -join ' | ')</p>" }
        
        if ($chkWebseite.Checked -and $textboxes[12].Text.Trim() -ne "") {
            $htmlBody += "<p>$($textboxes[12].Text)</p>"
        }
        
        if ($script:ShowLegal) {
            $disclaimerWrapped = Format-LineBreak $trans.Disclaimer 85
            $htmlBody += "<p style='font-size: 7pt; color: #666;'>$($disclaimerWrapped -replace "`n", "<br/>")</p>"
            $htmlBody += "<p style='font-size: 7pt; color: #FF1493;'>$("_" * 100)</p>"
            $htmlBody += "<p style='font-size: 7pt; color: #666;'>$($trans.Footer -replace "`n", "<br/>")</p>"
        }
